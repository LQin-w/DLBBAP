...
OPTION_TEXTS: dict[str, dict[str, dict[str, str]]] = {
    "zh": {
        ...
        "training.compile": {"name": "启用 torch.compile", "desc": "是否尝试对模型执行 torch.compile；若当前环境不支持，会记录原因并自动回退为未编译模型。双-backbone ensemble 下会保留安全保护逻辑，自动避开有风险的 CUDAGraphs 路径。"},
        "training.compile_mode": {"name": "编译模式", "desc": "torch.compile 的请求模式；UI 会按当前 torch build 展示可用模式。若为双-backbone ensemble，运行时会在日志中明确记录实际 backend/mode/options，并自动切到 no-cudagraphs 的安全等价路径。"},
    ...
    "en": {
        ...
        "training.compile": {"name": "Enable torch.compile", "desc": "Whether to attempt torch.compile for the model. If the current environment cannot use it, training falls back cleanly and logs the reason. Dual-backbone ensemble runs keep the safety guard and avoid risky CUDAGraphs paths automatically."},
        "training.compile_mode": {"name": "Compile Mode", "desc": "Requested torch.compile mode. The UI shows modes supported by the current torch build. For dual-backbone ensembles, runtime logs will state the effective backend/mode/options and automatically move to a safe no-cudagraphs equivalent path when needed."},
